...
powershell -ExecutionPolicy Bypass -File build.ps1                 # build FULL Tabel.exe (all 10 features, OCR)
powershell -ExecutionPolicy Bypass -File build.ps1 -Variant lite   # build LITE Tabel.exe (9 features, no proezd/cv2/winrt)
```

Two delivery variants are built from one codebase (Open/Closed): **full** (all 10 features incl. proezd + OCR, ~73 MB) and **lite** (9 features, excludes `app.features.proezd`/`cv2`/`winrt`/`numpy`, ~25 MB). `registry.py` registers features softly (try/except), so the lite build with proezd excluded just shows one card fewer; the other 9 (incl. grafiki/uslugi_dengi/gos_zadanie/proverka_kachestva/peresmotr) are picked up automatically by PyInstaller's static import analysis — no build.ps1 change needed when adding a feature that bundles no resources. Final packaging into a delivery folder (`Табель.exe` + `data/` + `Инструкция.txt`) and `.zip` is done outside the ASCII-only `build.ps1` (Cyrillic names); finished archives live in repo-root `Сборки/`.
