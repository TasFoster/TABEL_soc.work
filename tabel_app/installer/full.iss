; Inno Setup — Табель (полная версия). Файл в UTF-8 (BOM добавляется перед компиляцией).
; Версию можно переопределить при компиляции: ISCC.exe /DAppVer=1.3.0 full.iss
#ifndef AppVer
  #define AppVer "1.3.0"
#endif
#define SrcDir "C:\Users\User\tabel_pkg\full"
#define OutDir "C:\Users\User\tabel_pkg\out"

[Setup]
AppId={{7F3A2C10-9B4D-4E61-AE12-1F2D3C4B5A60}
AppName=Табель (полная)
AppVersion={#AppVer}
AppPublisher=Станислав
DefaultDirName={localappdata}\Tabel
DefaultGroupName=Табель
DisableProgramGroupPage=yes
PrivilegesRequired=lowest
OutputDir={#OutDir}
OutputBaseFilename=Табель_полная_setup
Compression=lzma2
...
[Icons]
Name: "{group}\Табель"; Filename: "{app}\Табель.exe"
Name: "{group}\Инструкция"; Filename: "{app}\Инструкция.txt"
Name: "{group}\Удалить Табель"; Filename: "{uninstallexe}"
Name: "{autodesktop}\Табель"; Filename: "{app}\Табель.exe"; Tasks: desktopicon
