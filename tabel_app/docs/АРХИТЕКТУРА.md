...
- **`clipboard.py`** — `enable_hotkeys(root)` (алиас `enable_cyrillic_clipboard`): горячие
  клавиши редактирования (Ctrl+C/V/X/A/Z/Y) на ЛЮБОЙ раскладке. На русской раскладке Tk
  выдаёт для физических клавиш не латинские keysym (и разные в разных сборках), поэтому
  штатные виртуальные события `<<Copy>>/…` не срабатывают. Решение — один общий классовый
  обработчик `<Control-KeyPress>` (на `Entry/TEntry/Spinbox/TSpinbox/TCombobox/Text`),
  диспетчеризующий по **keycode** (физическая клавиша, не зависит от раскладки), но только
  для НЕ-латинских keysym (для латиницы возвращает `None`, отдавая действие штатному Tk —
  без двойной вставки). Отмена/повтор/выделить-всё на латинице привязаны к штатным
  `<<Undo>>/<<Redo>>/<<SelectAll>>` (у Entry своей отмены нет — ведём стек снимков; у Text
  включаем родную `undo=True` при фокусе). Вызывается один раз в `shell.py` — на все окна.
...
| proverka_kachestva | `gui.py` (Treeview с инлайн-правкой; `PhonesManager` — редактор телефонов для «Справочников») | `service.py` (четверги месяца, разбор реестра) | `reestr/parser.py` (переиспользование, xlrd) / `writer.py` (odfpy, .ods с нуля, landscape) / `storage.py` (телефоны клиентов в БД `pk_phones`) |
...
| plany | `gui.py` (12 месяцев в Treeview, инлайн-правка соцработника) | `service.py` (сдвиг года единым offset от базы 2026 + подстановка соцработника) | `storage.py` (JSON-шаблоны задач `data/plany_templates.json` только чтение; соцработники в БД `plany_workers`) / `writer.py` (odfpy, **`.odt` OpenDocumentText** с нуля) |
